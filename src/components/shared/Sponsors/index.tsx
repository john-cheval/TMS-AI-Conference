...
SponsorsPropsType) => {
  ...
  // If more than 8 logos → split into 2 rows
  if (logos.length > 8) {
    const half = Math.ceil(logos.length / 2);

    const first = logos.slice(0, half);
    const second = logos.slice(half);

    return {
      firstList: [...first, ...first, ...first, ...first],
      secondList: [...second, ...second, ...second, ...second],
    };
  }

  // If <= 8 logos → single row
  return {
    firstList: [...logos, ...logos, ...logos, ...logos],
    secondList: [],
  };
}, [data?.data]);
  ...
  const shouldAnimateSecondList = listsData.secondList.length >= 5;
