  ...
  return (
    <section className="section-container">
      <div className="w-full">
        ...
        <div className="section-wrapper pt-8 md:pt-12 lg:pt-16 pb-8 sm:pb-0 border-b sm:border-b-0 border-[#bdbdbd] ">
          <h4 className="sub_heading-1">{keyBenifitsData?.heading}</h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 sm:gap-6 md:gap-8 lg:grid-cols-4 gap-x-10 xl:gap-x-16 3xl:gap-x-20 mt-5 sm:mt-6 md:mt-8">
            {keyBenifitsData?.all_benefits &&
              keyBenifitsData?.all_benefits?.length > 0 &&
              keyBenifitsData?.all_benefits?.map((item: any, index: number) => {
                return (
                  <div
                    key={index + 1}
